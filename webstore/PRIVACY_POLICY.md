...
Saved records remain in the encrypted local vault until the user confirms deletion of one record or clears the entire vault. Clearing the vault also removes its saved local API pairing token. Downloaded CSV and handoff files remain on disk until the user deletes them separately.
