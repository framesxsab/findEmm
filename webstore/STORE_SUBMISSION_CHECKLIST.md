...
- [x] Run `npm test`, `npm run check`, `npm run build`, and `npm run package:store` against the final source; verify the ZIP matches the tested build. Verified on 2026-07-19: 93 tests passed, syntax checks passed, authenticated local API health/screening and uncheckable-contact `422` smoke checks passed with integrations disabled, and all 11 packaged files matched `dist/extension` by SHA-256.
